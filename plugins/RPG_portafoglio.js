...
import { ensureRpgXp } from '../lib/rpg-xp.js'
...
const renderCard = async ({
  ...
  txt(
    `${Number(bank).toLocaleString('it-IT')} XP333`,
    x,
    cy + 28,
    '700 30px sans-serif',
    WHITE,
  ...
  txt(
    `${Number(wallet).toLocaleString('it-IT')} XP333`,
    x,
    cy + 110,
    '700 30px sans-serif',
    WHITE,
  ...
  txt(
    `${Number(stolen).toLocaleString('it-IT')} XP333`,
    x,
    cy + 188,
    '700 30px sans-serif',
    RED,
...
🏦 Banca: ${bank} XP333
👛 Tasca: ${money} XP333
💰 Totale: ${total.toLocaleString('it-IT')} XP333
🥷 Rubato: ${stolen} XP333
💳 Carta: ${user.card}`,
      m
    )
