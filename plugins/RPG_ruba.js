...
let handler = async (m, { conn }) => {
  ...
  if (senderTotalFunds < 1000)
    throw "🏦 𝐃𝐞𝐯𝐢 𝐚𝐯𝐞𝐫𝐞 𝐚𝐥𝐦𝐞𝐧𝐨 *1000 XP333* 𝐢𝐧 𝐛𝐚𝐧𝐜𝐚 𝐨 𝐧𝐞𝐥 𝐩𝐨𝐫𝐭𝐚𝐟𝐨𝐠𝐥𝐢𝐨 𝐩𝐞𝐫 𝐟𝐚𝐫𝐞 𝐮𝐧𝐚 𝐫𝐚𝐩𝐢𝐧𝐚!"
  ...
  const formatFineSource = ({ usedBank, usedMoney }) => {
    const parts = []
    if (usedBank > 0) parts.push(`*${usedBank} XP333* dalla banca`)
    if (usedMoney > 0) parts.push(`*${usedMoney} XP333* dal portafoglio`)
    return parts.join(' e ')
  }
  ...
  if (uVictim.money <= 0) {
    ...
    const sourceText = formatFineSource(paid) || '*0 XP333*'

    return conn.reply(
      m.chat,
      `🚨 @${who.split('@')[0]} 𝐧𝐨𝐧 𝐡𝐚 𝐬𝐨𝐥𝐝𝐢!\n𝐒𝐞𝐢 𝐬𝐭𝐚𝐭𝐨 𝐦𝐮𝐥𝐭𝐚𝐭𝐨 𝐝𝐢 *${multa} XP333* (${sourceText})`,
      null,
      { mentions: [who] }
    )
  }
  ...
  if (fallisce) {
    ...
    testo = `🚨 𝐒𝐞𝐢 𝐬𝐭𝐚𝐭𝐨 𝐬𝐜𝐨𝐩𝐞𝐫𝐭𝐨!\n𝐋𝐚 𝐩𝐨𝐥𝐢𝐳𝐢𝐚 𝐭𝐢 𝐡𝐚 𝐦𝐮𝐥𝐭𝐚𝐭𝐨 𝐝𝐢 *${multa} XP333* (${sourceText})`
  } else {
    ...
    testo = `💰 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐎!\n𝐇𝐚𝐢 𝐫𝐮𝐛𝐚𝐭𝐨 *${rubato} XP333* (${percentuale}%) 𝐚 @${who.split('@')[0]}`
  }
