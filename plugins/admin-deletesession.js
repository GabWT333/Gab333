...
const handler = async (message, { conn, isOwner, isROwner }) => {
  if (!isOwner && !isROwner) {
    return conn.sendMessage(message.chat, {
      text: "Comando reso solo per proprietari per evitare abusi, le sessioni si ripristinano da sole ogni mezz'ora."
    }, { quoted: message });
  }
