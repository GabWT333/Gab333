...
const ytDlpOptions = '--js-runtimes node --no-playlist'
...
`
    ...
    exec(`yt-dlp ${ytDlpOptions} -x --audio-format mp3 -o "${file}" ${video.url}`, async (err, stdout, stderr) => {

      if (err) {
        const details = [
          `Comando: yt-dlp ${ytDlpOptions} -x --audio-format mp3 -o "${file}" ${video.url}`,
          `Codice: ${err.code ?? 'sconosciuto'}`,
          stderr?.trim(),
          err.stack || err.message
        ].filter(Boolean).join('\n')

        console.error('Errore download audio:', details)
        return m.reply(`❌ Errore audio:\n\n${details}`)
      }
