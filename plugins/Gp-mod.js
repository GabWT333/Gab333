...
let handler = async (m, { conn, text, command, usedPrefix, isOwner, isAdmin }) => {
    ...
    if (command === 'addmod') {
        ...
        mods.push(who)
        return m.reply(`✅ @${who.split('@')[0]} aggiunto!\nPuò usare i comandi admin, tranne gestire i ruoli.\n> *Digita ''.mods'' per vedere la lista e i comandi per i moderatori*\n\n> *Digita ''delmodall'' per rimuovere tutti i moderatori*`, null, { mentions: [who] })
    }
    ...
    if (command === 'delmodall') {
        if (!mods || mods.length === 0) return m.reply("📋 Nessun moderatore registrato.")
        const removed = mods.slice()
        global.db.data.chats[chatId].moderatori = []
        return m.reply(`🗑️ Tutti i moderatori sono stati rimossi (${removed.length}):\n${removed.map(j => '@' + j.split('@')[0]).join(' ')}`, null, { mentions: removed })
    }
...
handler.before = async function (m) {
    ...
    const comandiProibiti = /^(p|promote|r|retrocedi|promuovi|demote|unadmin|addadmin|deladmin)$/i
...
handler.help = ['addmod', 'delmod', 'delmodall', 'listanera']
handler.tags = ['admin', 'group']
handler.command = /^(addmod|delmod|delmodall|listanera)$/i
handler.group = true
handler.admin = true
