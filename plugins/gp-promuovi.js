...
let handler = async (m, { conn, text }) => {
    ...
    if (!global.pendingAdminActor) global.pendingAdminActor = new Map()
    const pendingKey = `${m.chat}|promote`
    global.pendingAdminActor.set(pendingKey, { actor: m.sender })
    setTimeout(() => global.pendingAdminActor.delete(pendingKey), 15000)
