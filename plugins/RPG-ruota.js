...
let handler = async (m, { conn, command }) => {
  ...
  if (command === 'ruota') {
    const rateCheck = checkRateLimit(m.sender, 'ruota')
    if (!rateCheck.allowed) {
      return m.reply(`⏳ *Devi aspettare ${rateCheck.timeLeft} secondi* prima di giocare di nuovo alla ruota!`)
    }
...
┃ ⏳𝐇𝐚𝐢 𝟑𝟎 𝐬𝐞𝐜𝐨𝐧𝐝𝐢 𝐝𝐢 𝐭𝐞𝐦𝐩𝐨\n┃ 𝐩𝐞𝐫 𝐫𝐢𝐬𝐩𝐨𝐧𝐝𝐞𝐫𝐞\n┃━━━━━━━━━━━━━━\n> 𝐋𝐚 𝐫𝐮𝐨𝐭𝐚 𝐬𝐢 𝐩𝐮𝐨̀ 𝐠𝐢𝐫𝐚𝐫𝐞 𝐦𝐚𝐬𝐬𝐢𝐦𝐨 𝟑 𝐯𝐨𝐥𝐭𝐞 𝐚𝐥 𝐠𝐢𝐨𝐫𝐧𝐨!\n╰─────────╯`,
  ...
  if (command === 'si') {
    ...
    const premi = [
      { name: '*100 XP333*', amount: 100, weight: 20 },
      { name: '*200 XP333*', amount: 200, weight: 15 },
      { name: '*300 XP333*', amount: 300, weight: 10 },
      { name: '*400 XP333*', amount: 400, weight: 8 },
      { name: '*500 XP333*', amount: 500, weight: 5 },
      { name: '*1000 XP333*', amount: 1000, weight: 4 },
      { name: '*Raddoppia*', amount: 'double', weight: 3 },
      { name: '*5000 XP333*', amount: 5000, weight: 2 },
      { name: '*10.000 XP333*', amount: 10000, weight: 1 },
      { name: '*100.000 XP333*', amount: 100000, weight: 0.5 }
    ]
    ...
    const perdite = [
      ...
      { name: '*-500 XP333*', amount: -500, weight: 5 },
      { name: '*-1000 XP333*', amount: -1000, weight: 4 },
      { name: '*-10.000 XP333*', amount: -10000, weight: 2 },
      { name: '*PERDI TUTTO XP333*', amount: 'all', weight: 1 }
    ]
    ...
    const xpChange = risultato.amount === 'all'
      ? -25
      : (risultato.amount === 'double' || risultato.amount > 0 ? 20 : -10)
    addRpgXp(user, xpChange)

    if (user.bank < 0) user.bank = 0

    let finalText = (risultato.amount > 0 || risultato.amount === 'double') ?
      `🏆 𝐕𝐈𝐓𝐓𝐎𝐑𝐈𝐀!\n𝐇𝐚𝐢 𝐯𝐢𝐧𝐭𝐨: ${risultato.name}\n⚡ XP333: +${xpChange}\n💰 𝐒𝐚𝐥𝐝𝐨 𝐛𝐚𝐧𝐜𝐚𝐫𝐢𝐨 𝐚𝐭𝐭𝐮𝐚𝐥𝐞: *${user.bank} XP333*\n𝐅𝐨𝐫𝐭𝐮𝐧𝐚𝐭𝐨!` :
