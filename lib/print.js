...
export default async function (m, conn = { user: {} }) {
  if (!m || m.key?.fromMe || m.fromMe || m.type === 'protocol' || m.messageStubType || !m.message || m.mtype === 'protocolMessage') return;
  ...
  try {
    const rawSenderJid = m?.key?.participant || m?.sender || m?.key?.participantPn || ''
    let senderJid = rawSenderJid
    const botJidRaw = conn.user?.jid || conn.user?.id || ''
    const botDigits = String(botJidRaw).split(':')[0].replace(/\D/g, '')
    const isBotDigits = (jid) => {
      if (!jid || !botDigits) return false
      const d = String(jid).split('@')[0].replace(/:\d+$/, '').replace(/\D/g, '')
      return d.length >= 8 && d === botDigits
    }
    const physicalLidSender = typeof rawSenderJid === 'string' && rawSenderJid.endsWith('@lid')
    if (physicalLidSender && !m.key?.fromMe) {
      senderJid = rawSenderJid.includes(':') ? rawSenderJid.replace(/:\d+@/, '@') : rawSenderJid
    } else if (senderJid && typeof senderJid === 'string' && senderJid.endsWith('@lid')) {
      try {
        
        if (!m.key?.fromMe) {
          const participants = m?.isGroup ? (groupMetaCache.get(m.chat || m.key?.remoteJid)?.participants || []) : [];
          const resolved = global.resolveLidToJid ? global.resolveLidToJid(senderJid, conn, participants) : senderJid;
          if (resolved && !String(resolved).endsWith('@lid') && !isBotDigits(resolved)) {
            senderJid = resolved
          }
          
        } else {
          const participants = m?.isGroup ? (groupMetaCache.get(m.chat || m.key?.remoteJid)?.participants || []) : [];
          senderJid = global.resolveLidToJid ? global.resolveLidToJid(senderJid, conn, participants) : senderJid;
        }
      } catch {
      }
    }
    if (!senderJid || senderJid.endsWith('@lid')) {
      ...
      const op = m.key?.participant
      if (op && String(op).endsWith('@lid')) senderJid = String(op).includes(':') ? String(op).replace(/:\d+@/, '@') : String(op)
      else senderJid = rawSenderJid
    } else {
    ...
    if (!physicalLidSender && !m.key?.fromMe && (senderJid === botJid || isBotDigits(senderJid))) {
      const originalParticipant = m.key?.participant || m.key?.participantLid || m.key?.participantPn
      if (typeof originalParticipant === 'string' && originalParticipant) {
        if (originalParticipant.endsWith('@lid')) {
          senderJid = originalParticipant.includes(':') ? originalParticipant.replace(/:\d+@/, '@') : originalParticipant
        } else if (!isBotDigits(originalParticipant)) {
          senderJid = conn.decodeJid(originalParticipant)
        }
      }
    }
    ...
    const getName = async (jid) => {
      ...
      let lookup = jid;
      try {
        if (jid.endsWith('@lid') && global.lidCache) {
          const v = global.lidCache.get(jid);
          if (v && !String(v).endsWith('@lid')) {
            lookup = String(v).includes('@') ? String(v).replace(/:\d+@/, '@') : (String(v).replace(/\D/g, '') + '@s.whatsapp.net');
            const c2 = nameCache.get(lookup);
            if (c2 && c2 !== 'Utente sconosciuto') return c2;
          }
        }
      } catch {}

      if (lookup.endsWith('@newsletter') || jid.endsWith('@newsletter')) {
        cached = 'Newsletter ' + jid.split('@')[0];
      } else if (lookup.endsWith('@g.us') || jid.endsWith('@g.us')) {
        const meta = groupMetaCache.get(lookup) || groupMetaCache.get(jid);
        cached = meta?.subject || '';
      }

      if (!cached) {
        const c = conn.contacts?.[lookup] || conn.contacts?.[jid] || global.store?.contacts?.[lookup] || global.store?.contacts?.[jid];
        cached = c?.notify || c?.name || c?.verifiedName || '';
        if (!cached) {
          try { cached = await conn.getName(lookup) || await conn.getName(jid) || ''; } catch { cached = ''; }
        }
      }

      if (!cached || cached === 'Utente sconosciuto' || cached === 'Sconosciuto') {
        cached = jid.endsWith('@g.us') ? 'Chat' : 'Utente';
      }

      nameCache.set(jid, cached);
      if (lookup !== jid) nameCache.set(lookup, cached);
      return cached;
    };
    ...
    const senderPhone = (senderJid.endsWith('@s.whatsapp.net') || senderJid.endsWith('@c.us'))
      ? senderJid.split('@')[0].replace(/:\d+$/, '').replace(/\D/g, '')
      : '';
    const botPhone = botJid?.split('@')[0].replace(/:\d+$/, '').replace(/\D/g, '') || '';
    const isOwner = Boolean(senderPhone && senderPhone !== botPhone && (global.owner || []).some(entry => {
      const number = Array.isArray(entry) ? entry[0] : entry;
      return String(number || '').replace(/[^0-9]/g, '') === senderPhone;
    }));
    const isGroup = chatJid.endsWith('@g.us');
    const isAdmin = isGroup ? await checkAdmin(conn, chatJid, senderJid) : false;
    const numeroMittenteLog = String(m.sender || '').replace(/[^0-9]/g, '')
    const numeroBotLog = String(conn.user?.jid || '').replace(/[^0-9]/g, '')
    const eIlBotReale = numeroMittenteLog === numeroBotLog && numeroMittenteLog.length > 0;
    
    const isSenderLid = String(m.sender || '').endsWith('@lid');
    const isFromMeFalse = m.fromMe === false;
    const shouldExcludeBot = isSenderLid || (isFromMeFalse && !m.key?.fromMe);
    const role = isOwner ? 'OWNER' : (shouldExcludeBot ? (isAdmin ? 'ADMIN' : 'USER') : (eIlBotReale ? 'BOT' : (isAdmin ? 'ADMIN' : 'USER')));
  ...
  `\x1b[38;5;196m║\x1b[0m \x1b[38;5;46mUTENTE\x1b[0m ⇢ \x1b[37m${sender}\x1b[0m${isGroup ? ` \x1b[90m⟡ ${getUserStatus(role, isPremium, isBanned, c)}\x1b[0m` : ''}`,
...
function formatPhoneNumber(jid, name) {
  ...
  if (jid.endsWith('@lid')) {
    const lidCode = jid.split('@')[0].replace(/:\d+$/, '').replace(/\D/g, '');
    return `${lidCode || jid.split('@')[0]}${name ? ` ~${name}` : ''}`;
  }
...
async function checkAdmin(conn, chatId, senderId) {
  try {
    ...
    const senderNumber = String(senderId || '').replace(/[^0-9]/g, '')
    if (!senderNumber) return false
    return groupMeta.participants.some(p =>
      (p.admin === 'admin' || p.admin === 'superadmin' || p.admin === true) &&
      String(p.id || '').replace(/[^0-9]/g, '') === senderNumber
    ) || false
  } catch {
    return false
  }
...
function getUserStatus(role, isPremium, isBanned, c) {
  let status = []
  if (role === 'OWNER') status.push(c.success('👑 OWNER'))
  if (role === 'BOT') status.push(c.success('BOT'))
  if (role === 'ADMIN') status.push(c.warning('ADMIN'))
    ...
    for (const id of m.mentionedJid) {
      try {
        ...
        if (!name) {
          ...
          if (!name || name === 'Utente sconosciuto' || name === 'Sconosciuto') name = displayNum || 'Utente';
