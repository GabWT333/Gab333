...
let handler = async (m, { conn, text, command }) => {
  ...
  if (command === "note") {
    ...
    return conn.sendMessage(m.chat, {
      ...
      buttons: [
        ...
        { buttonId: ".delnotetutte", buttonText: { displayText: "🗑️ Rimuovi tutte le note" }, type: 1 }
      ],
      headerType: 1
    }, { quoted: m })
  }
  ...
  if (command === "delnotetutte") {
    if (user.note.length === 0)
      return m.reply("📭 Non hai note da eliminare")

    const rimosse = user.note.length
    user.note = []

    return m.reply(`🗑️ Rimosse tutte le note (${rimosse})`)
  }
}

handler.command = /^(nota|note|rimuovinota|delnota|delnotetutte)$/i
